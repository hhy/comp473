...
public class MyPaint extends JPanel implements MouseListener,
		MouseMotionListener {
	...
	public void paintComponent(Graphics g) {
		 ...
		 * right corner of the applet, allowing for a 3-pixel border.
		 */
		...
		g.setColor(Color.WHITE);
		g.drawString("Black", width - 50, 50);
		...
		g.setColor(Color.WHITE);
		g.drawRect(width - 55, 1 + currentColor * colorSpacing, 53,
				colorSpacing);
		g.drawRect(width - 54, 2 + currentColor * colorSpacing, 51,
				colorSpacing - 2);
	 ...
	 * drawing a curve, the curve is done, so we should set drawing to false and
	 * get rid of the graphics context that we created to use during the
	 * drawing.
	 */
	 ...
	 * down. If the user is drawing, draw a line segment from the previous mouse
	 * location to the current mouse location, and set up prevX and prevY for
	 * the next call. Note that in case the user drags outside of the drawing
	 * area, the values of x and y are "clamped" to lie within this area. This
	 * avoids drawing on the color palette or clear button.
	 */
