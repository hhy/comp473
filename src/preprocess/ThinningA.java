...
public class ThinningA {

	/**
	 * Gets the 8 circular neighbors taken clockwise starting from the up
	 * neighbor until the up-left one.
	 */
	...
	/**
	 * Returns the number of neighbor pixels that belong to the shape
	 * represented as a boolean array in isShape
	 * 
	 * @param isShape
	 *            binary array that is true for the pixels that belong to the
	 *            shape
	 * @param neighbors
	...
	/**
	 * Count the number of false-true (01) patterns clockwise. The neighbors
	 * array must be clockwise starting from up,just as the getCircularNeighbors
	 * of this class returns.
	 * 
	 * @param isShape
	 *            isShape binary array that is true for the pixels that belong
	 *            to the shape
	 * @param neighbors
	 ...
	 *            shape
	 * @param w
	 *            Width of the image from which the distance transformation was
	 *            done
	 * @param h
	 *            Height of the image from which the distance transformation was
	 *            done
	 * @param shapePoints
	 *            list containing the positions for the pixels that belong to
	...
	public static boolean thinContour(int[] dtImage, boolean[] isShape, int w,
		...
		while (it.hasNext()) {
			...
			n = ThinningA.getCircularNeighbors(count, w);
			b = ThinningA.nonZeroNeighbors(isShape, n);
			a = ThinningA.circular01Patterns(isShape, n);
	...
	/**
	 * Applies T.Y. Zhang and C.Y. Suen thinning-skeletonization algorithm
	 * iteratively calling the thinContour method,until reaching a 1-pixel width
	 * image.
	 * 
	 * @param dtImage
	 *            distance transformation image
	...
	public static void thinToSkeleton(int[] dtImage, boolean[] isShape, int w,
		...
		int contourIndex = 1;
		while (makeThinner) {
			makeThinner = ThinningA.thinContour(dtImage, isShape, w, h,
					shapePoints, contourIndex);
			contourIndex++;
		}
		contourIndex = Integer.MAX_VALUE;

		ThinningA
				.thinContour(dtImage, isShape, w, h, shapePoints, contourIndex);
	}
}
